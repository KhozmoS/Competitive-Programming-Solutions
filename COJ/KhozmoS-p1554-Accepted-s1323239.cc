...
// O(n log n)
vector<Point> ConvexHull_fast( vector<Point> P ) {
...
int main()
{
   ...
   while( t-- ) {
        ...
        int pos = 0;
        double mx = 1e14 , my = 1e14;
        ans = ConvexHull_fast( v );
        for(int i = 0 ; i < ans.size() ; i++)
        {
            if( ans[i].y < my || (ans[i].y==my && ans[i].x < mx)) {
                pos = i;
                mx = ans[i].x;
                my = ans[i].y;
            }
        ...
        int cnt = 0;
        cout<<ans.size()+1<<"\n";
        for(int i = pos ; cnt <= ans.size() ; i++ , cnt++)
            cout<<(int)ans[i%(int)(ans.size())].x<<" "<<(int)ans[i%(int)(ans.size())].y<<"\n";
        if( t )
        {
            cout<<neg<<"\n";
        }
