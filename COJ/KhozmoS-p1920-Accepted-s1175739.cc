...
void lazy(int idx, int node, int ini, int fin){
    ...
    lz[idx][n1] += lz[idx][node];
    lz[idx][n2] += lz[idx][node];

    ST[idx][n1] += lz[idx][node];
    ST[idx][n2] += lz[idx][node];
...
void update(int idx , int node , int ini , int fin){
    ...
    ST[idx][node] = max(ST[idx][n1], ST[idx][n2]);
...
int solve_q(int a , int b)
{
    ...
    while(cad[a] != cad[b])
    {
        ...
        in = pos[c];
        fn = pos[a];
        res = max(res , query(cad[a] , 1 , 1 , len_cad[cad[a]]));
        a = p[c];
    }
